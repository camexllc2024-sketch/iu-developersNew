...
app.listen(PORT, () => {
  console.log(`Email server with fallback running on port ${PORT}`);
  console.log(`Primary SMTP: ${process.env.SMTP_HOST}:${process.env.SMTP_PORT}`);
  console.log(`Gmail fallback: ${gmailTransporter ? 'configured' : 'not configured'}`);
});
